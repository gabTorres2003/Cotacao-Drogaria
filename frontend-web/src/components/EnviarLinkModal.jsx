...
export default function EnviarLinkModal({ idCotacao, onClose }) {
  ...
  useEffect(() => {
    api.get('/api/fornecedor')
       ...
       .catch(err => {
         console.error(err);
         alert("Erro ao buscar fornecedores. Verifique se o Backend está rodando.");
         setLoading(false);
       });
  }, []);
  ...
  const enviarPara = async (fornecedor) => {
    ...
    try {
      ...
      alert("Erro ao gerar link: " + (error.response?.data || "Erro desconhecido"));
    }
  };
              ...
              {fornecedores.map(f => (
                <button 
                  ...
                  style={{
                    ...
                    padding: '10px', border: '1px solid #eee', borderRadius: '6px',
